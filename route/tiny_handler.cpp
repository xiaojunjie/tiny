...
tiny_int_t tiny_http_parse_handler(const tiny_string_t &buf,
    ...
    if (j < 0)
        return TINY_ERROR;
    tiny_string_t request_line = buf.substr(0, i);
    tiny_string_t header = buf.substr(i + 2, j - i);
    tiny_string_t body = buf.substr(j + 4);
    return http_parse_request_line(request_line, request) &&
